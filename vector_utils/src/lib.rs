...
pub fn bin_position12_3<S: Ord, T: Ord, U: Ord>(x: &[(S, T, U)], d: (&S, &T)) -> i32 {
    match x.binary_search_by_key(&d, |&(ref a, ref b, ref _c)| (a, b)) {
        Ok(p) => p as i32,
        Err(_e) => -1,
    }
}
...
pub fn next_diff12_6<S: Eq, T: Eq, U: Eq, V: Eq, W: Eq, X: Eq>(
    x: &[(S, T, U, V, W, X)],
    i: i32,
) -> i32 {
    let mut j: i32 = i + 1;
...
#[allow(clippy::type_complexity)]
pub fn next_diff12_7<S: Eq, T: Eq, U: Eq, V: Eq, W: Eq, X: Eq, Y: Eq>(
    x: &[(S, T, U, V, W, X, Y)],
    i: i32,
) -> i32 {
    let mut j: i32 = i + 1;
...
pub fn sort_sync_by2<T: PartialOrd + Clone, S1: PartialOrd + Clone>(
    t: &mut Vec<T>,
    s1: &mut Vec<S1>,
) {
...
pub fn reverse_sort_sync_by2<T: PartialOrd + Clone, S1: PartialOrd + Clone>(
    ...
    let permutation = permutation::sort_by(&t[..], |a, b| b.partial_cmp(a).unwrap());
    *t = permutation.apply_slice(&t[..]);
    *s1 = permutation.apply_slice(&s1[..]);
}
...
pub fn reverse_sort_sync3<T: Ord + Clone, S1: Ord + Clone, S2: Ord + Clone>(
    t: &mut Vec<T>,
    s1: &mut Vec<S1>,
    s2: &mut Vec<S2>,
) {
...
pub fn reverse_sort_sync4<T: Ord + Clone, S1: Ord + Clone, S2: Ord + Clone, S3: Ord + Clone>(
    t: &mut Vec<T>,
    s1: &mut Vec<S1>,
    s2: &mut Vec<S2>,
    s3: &mut Vec<S3>,
