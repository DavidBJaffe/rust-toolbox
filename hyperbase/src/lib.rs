...
impl Hyper {
    ...
    // =============================================================================
    // Create a new Hyper from data.  K=40 instead.  Ugh, duplication.
    // =============================================================================

    pub fn build_from_reads_40(&mut self, k: i32, reads: &[DnaString]) {
        // Only works if k = 40.
        ...
        let mut kmers_plus = Vec::<(Kmer40, i32, i32)>::new();
        let mut edges = Vec::<DnaString>::new();
        for e in 0..self.h.g.edge_count() {
            edges.push(self.h.g.edge_obj(e as u32).clone());
        }
        make_kmer_lookup_40_single(&edges, &mut kmers_plus);
        drop(edges);
        let mut maxread = 0;
        for id in 0..reads.len() {
            maxread = max(maxread, reads[id].len());
        ...
        for pos in 0..maxread - (k as usize) + 1 {
            for id in 0..reads.len() {
                ...
                let x: Kmer40 = reads[id].get_kmer(pos);
                let p = bin_position1_3(&kmers_plus, &x);
                if p < 0 {
                    continue;
                }
